...
public class PausedQuarters extends AbstractShape implements Shape {

	public PausedQuarters() {
		name = "PausedQuarter"; 
		period = Math.PI * 4;
	}
	...
	public Point draw(double length, double circleModifier, double realCircleAngle,  double circleTurnModifier) {
		...
		{   // 2 -> 3 pause 2 
			...
			if (realCircleAngle < 0)  y = ((length/2) * circleModifier) * -1 * Math.sin((Math.PI / 2)); 
		...
		{   // 5 -> 6 move 3 
			x = ((length/2) * circleModifier) * Math.cos((realCircleAngle - (Math.PI * 3) ));  
			y = ((length/2) * circleModifier) * Math.sin((realCircleAngle - (Math.PI * 3) )); 
		}
		else if(circleAngle > (Math.PI * 4) + (Math.PI / 2) && circleAngle <= ((Math.PI * 5) + (Math.PI / 2))) 
		{   // 4 -> 5 pause 4
			x = ((length/2) * circleModifier) * Math.cos((Math.PI / 2));  
			if (realCircleAngle < 0)  y = ((length/2) * circleModifier) * Math.sin((Math.PI / 2)); 
			else y = ((length/2) * circleModifier) * -1 * Math.sin((Math.PI / 2)); 
		}
		else if(circleAngle > (Math.PI * 5) + (Math.PI / 2) && circleAngle <= ((Math.PI * 6))) 
		{   // 5 -> 6 move 4 
			x = ((length/2) * circleModifier) * Math.cos((realCircleAngle - ((Math.PI * 4))));  
			y = ((length/2) * circleModifier) * Math.sin((realCircleAngle - ((Math.PI * 4)))); 
		}
