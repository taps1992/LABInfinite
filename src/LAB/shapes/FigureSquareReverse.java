...
public class FigureSquareReverse extends AbstractShape implements Shape{
	
	
	public FigureSquareReverse() {
		name = "FigureSquare"; 
		period = Math.PI * 8.0;
	}
	...
	public Point draw(double length, double circleModifier, double realCircleAngle, double circleTurnModifier) {
		...
		else if( circleAngle >= ((Math.PI) + (Math.PI / 2)) &&  circleAngle < (Math.PI * 4)) {
